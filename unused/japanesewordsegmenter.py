...
# -*- coding: utf-8 -*-
#
# Originally written in JavaScript, found at:
...
#
# Ported to Python by Rezual (2018)
...
class TinySegmenter(object):
  # Use a tuple in Python, to match the JavaScript version's {} order
  patterns = (
    (r"[一二三四五六七八九十百千万億兆]", "M"),
    (r"[一-龠々〆ヵヶ]", "H"),
    (r"[ぁ-ん]", "I"),
  ...
  chartype_ = []
  for (find_pattern, replace_with) in patterns:
    regexp = re.compile(find_pattern)
    chartype_.append([regexp, replace_with])
  ...
  @classmethod
  def ctype_(cls, content):
    for i in cls.chartype_:
      if (i[0].search(content)):
        return i[1]

    return "O"

  @classmethod
  def segment(cls, input):
    if (input in (None, "")):
      return []
    ...
    for i in range(4, len(seg) - 3):
      ...
      score += cls.UP1__.get(p1, 0)
      score += cls.UP2__.get(p2, 0)
      score += cls.UP3__.get(p3, 0)
      score += cls.BP1__.get(p1 + p2, 0)
      score += cls.BP2__.get(p2 + p3, 0)
      score += cls.UW1__.get(w1, 0)
      score += cls.UW2__.get(w2, 0)
      score += cls.UW3__.get(w3, 0)
      score += cls.UW4__.get(w4, 0)
      score += cls.UW5__.get(w5, 0)
      score += cls.UW6__.get(w6, 0)
      score += cls.BW1__.get(w2 + w3, 0)
      score += cls.BW2__.get(w3 + w4, 0)
      score += cls.BW3__.get(w4 + w5, 0)
      score += cls.TW1__.get(w1 + w2 + w3, 0)
      score += cls.TW2__.get(w2 + w3 + w4, 0)
      score += cls.TW3__.get(w3 + w4 + w5, 0)
      score += cls.TW4__.get(w4 + w5 + w6, 0)
      score += cls.UC1__.get(c1, 0)
      score += cls.UC2__.get(c2, 0)
      score += cls.UC3__.get(c3, 0)
      score += cls.UC4__.get(c4, 0)
      score += cls.UC5__.get(c5, 0)
      score += cls.UC6__.get(c6, 0)
      score += cls.BC1__.get(c2 + c3, 0)
      score += cls.BC2__.get(c3 + c4, 0)
      score += cls.BC3__.get(c4 + c5, 0)
      score += cls.TC1__.get(c1 + c2 + c3, 0)
      score += cls.TC2__.get(c2 + c3 + c4, 0)
      score += cls.TC3__.get(c3 + c4 + c5, 0)
      score += cls.TC4__.get(c4 + c5 + c6, 0)
      #score += cls.TC5__.get(c4 + c5 + c6, 0)
      score += cls.UQ1__.get(p1 + c1, 0)
      score += cls.UQ2__.get(p2 + c2, 0)
      score += cls.UQ3__.get(p3 + c3, 0)
      score += cls.BQ1__.get(p2 + c2 + c3, 0)
      score += cls.BQ2__.get(p2 + c3 + c4, 0)
      score += cls.BQ3__.get(p3 + c2 + c3, 0)
      score += cls.BQ4__.get(p3 + c3 + c4, 0)
      score += cls.TQ1__.get(p2 + c1 + c2 + c3, 0)
      score += cls.TQ2__.get(p2 + c2 + c3 + c4, 0)
      score += cls.TQ3__.get(p3 + c1 + c2 + c3, 0)
      score += cls.TQ4__.get(p3 + c2 + c3 + c4, 0)
...
if __name__ == "__main__":
  ...
  segs = segmenter.segment("私の名前はリトルセグメンター") 
