...
#     Update: Ended up just rephrasing the English to be shorter in this case
...
# Load the translations that are applied everywhere
...
for line in eboot_translations:
    ...
    if not translation:
        ...
        translation = translation.get("guess") or translation.get("machine_deepl") or translation.get("machine_google")
...
# Patch start of Memcpy, to have it use Memcpy on translated string
# instead of the original in ARG1
NEW_MEMCPY_START_ADDRESS = NEW_STRLEN_ADDRESS
NEW_MEMCPY_ADDRESS = NEW_MEMCPY_START_ADDRESS

cwcheat(NEW_MEMCPY_ADDRESS, 0x27BDFFFC); NEW_MEMCPY_ADDRESS += 4 # addiu sp,sp,-0x4
cwcheat(NEW_MEMCPY_ADDRESS, 0xAFBF0000); NEW_MEMCPY_ADDRESS += 4 # sw ra, 0x0(sp)
cwcheat(NEW_MEMCPY_ADDRESS, 0x0C000000 | ((LOOKUP_ARG1_START_ADDRESS & 0x0FFFFFFF) >> 2)); NEW_MEMCPY_ADDRESS += 4 # memcpy: jal LOOKUP_ARG1_START_ADDRESS
cwcheat(NEW_MEMCPY_ADDRESS, 0x00000000); NEW_MEMCPY_ADDRESS += 4 # nop
cwcheat(NEW_MEMCPY_ADDRESS, 0x8FBF0000); NEW_MEMCPY_ADDRESS += 4 # lw ra, 0x0(sp)
cwcheat(NEW_MEMCPY_ADDRESS, 0x27BD0004); NEW_MEMCPY_ADDRESS += 4 # addiu sp,sp,0x4
cwcheat(NEW_MEMCPY_ADDRESS, 0x00A41025); NEW_MEMCPY_ADDRESS += 4 # or v0, a1, a0 # overwritten
cwcheat(NEW_MEMCPY_ADDRESS, 0x30420003); NEW_MEMCPY_ADDRESS += 4 # andi v0,v0,0x3 # overwritten
cwcheat(NEW_MEMCPY_ADDRESS, 0x0A26920A); NEW_MEMCPY_ADDRESS += 4 # j 0x089A4828
cwcheat(NEW_MEMCPY_ADDRESS, 0x00000000); NEW_MEMCPY_ADDRESS += 4 # nop

cwcheat(0x089A4820, 0x08000000 | ((NEW_MEMCPY_START_ADDRESS & 0x0FFFFFFF) >> 2)) # memcpy: j NEW_MEMCPY_ADDRESS
cwcheat(0x089A4824, 0x00000000) # nop

# Patch start of Memmove, to have it use Memmove on translated string
# instead of the original in ARG1
NEW_MEMMOVE_START_ADDRESS = NEW_MEMCPY_ADDRESS
...
# Quick variable to simplify calculating the branches
# 445 should be the the line number of the assert
D = (445 - __line__() - 2)
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12400000 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, zero, EIGHT
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x34060066    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x66 # f
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460004 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, SIX
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x34060069    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x69 # i
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460002 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, FOUR
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x3406006A    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x6A # j
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460002 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, FOUR
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x3406006C    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x6C # l
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460002 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, FOUR
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x3406006D    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x6D # m
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460006 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, TWELVE
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x3406006F    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x6F # o
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460000 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, EIGHT
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x34060072    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x72 # r
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460004 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, SIX
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x34060074    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x74 # t
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460004 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, SIX
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x34060027    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x27 # '
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460002 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, FOUR
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x34060049    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x49 # I
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460004 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, SIX
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x3406004D    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # li a2, 0x4D # M
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x12460006 + D); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # beq s2, a2, TWELVE
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x00000000    ); NEW_HALFWIDTH_ADDRESS += 4; D -= 1 # nop
assert D == -1, "By the end of this block, D should be 0. If not, the initial value had the wrong line number. D instead is: %s" % D

cwcheat(NEW_HALFWIDTH_ADDRESS, 0x10000007); NEW_HALFWIDTH_ADDRESS += 4 # b DONE (LABEL: EIGHT / DEFAULT) # 0
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x34130008); NEW_HALFWIDTH_ADDRESS += 4 # li s3, 0x08
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x10000005); NEW_HALFWIDTH_ADDRESS += 4 # b DONE (LABEL: FOUR) # 2
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x34130004); NEW_HALFWIDTH_ADDRESS += 4 # li s3, 0x04
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x10000003); NEW_HALFWIDTH_ADDRESS += 4 # b DONE (LABEL: SIX) # 4
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x34130006); NEW_HALFWIDTH_ADDRESS += 4 # li s3, 0x06
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x10000001); NEW_HALFWIDTH_ADDRESS += 4 # b DONE (LABEL: TWELVE) # 6
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x3413000B); NEW_HALFWIDTH_ADDRESS += 4 # li s3, 0x0B
cwcheat(NEW_HALFWIDTH_ADDRESS, 0x0A21CFE8); NEW_HALFWIDTH_ADDRESS += 4 # j 0x08873FA0 # (LABEL: DONE)
...
# Patch: Prevent crashes if text that's too long is in a message box
# There's a draw single character routine at 0x08873f14 that keeps
# writing to a buffer and incrementing a counter as much as it's called
# The patch below will stop incrementing if it's reached the end
NEW_CRASH_PREVENTION_START_ADDRESS = NEW_HALFWIDTH_ADDRESS
NEW_CRASH_PREVENTION_ADDRESS = NEW_CRASH_PREVENTION_START_ADDRESS

cwcheat(NEW_CRASH_PREVENTION_ADDRESS, 0x298B005B); NEW_CRASH_PREVENTION_ADDRESS += 4 # slti t3,t4,0x5B
cwcheat(NEW_CRASH_PREVENTION_ADDRESS, 0x018B5820); NEW_CRASH_PREVENTION_ADDRESS += 4 # add t3,t4,t3
cwcheat(NEW_CRASH_PREVENTION_ADDRESS, 0x0A21D025); NEW_CRASH_PREVENTION_ADDRESS += 4 # j 0x08874094
cwcheat(NEW_CRASH_PREVENTION_ADDRESS, 0xA62B001A); NEW_CRASH_PREVENTION_ADDRESS += 4 # sh t3, 0x1A(s1)

cwcheat(0x0887408C, 0x08000000 | ((NEW_CRASH_PREVENTION_START_ADDRESS & 0x0FFFFFFF) >> 2)) # j NEW_CRASH_PREVENTION_START_ADDRESS
cwcheat(0x08874090, 0x00000000) # nop
